 ...
 *   1. Raw .splat binary rows (32 bytes/point: pos f32Ã—3, scale f32Ã—3,
 *      color u8Ã—4, rotation u8Ã—4 â€” the standard antimatter15 .splat layout
 *      used by splat-edit-engine.js / window.S.sourceBuffer).
 *   2. Parsed PLY point clouds (position + color + opacity only, no
 *      per-point scale/rotation â€” isotropic points).
 *
 ...
 * opacity_logit, color_logit.rgb] â€” raw float32 format (flag 0x00).
 * â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
 */
...
// â”€â”€ logit / sigmoid helpers (canonical NIF color+opacity space) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function logit(p) {
...
// â”€â”€ Encode: 32-byte .splat rows â†’ NIF geometry Float32Array â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
/**
...
function splatRowsToGeometry(sourceBuffer, aliveIndices, rowSize = 32) {
  ...
  aliveIndices.forEach((srcIdx, row) => {
    ...
    // Color (4 Ã— uint8: r,g,b,a) at offset 24 â€” a doubles as opacity
    ...
    // Rotation quaternion (4 Ã— uint8) at offset 28, antimatter15 convention:
...
// â”€â”€ Encode: plain point cloud (PLY case â€” no scale/rotation) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
/**
 * @param {Float32Array} positions   n Ã— 3
 * @param {Float32Array} colors01    n Ã— 3, each channel 0â€“1
 * @param {Float32Array} opacities01 n, 0â€“1
 */
function plyPointsToGeometry(positions, colors01, opacities01) {
  const count = opacities01.length;
  const out   = new Float32Array(count * FLOATS_PER_POINT);
  // No real scale/rotation data for plain point clouds â€” encode as small
...
// â”€â”€ Quantized geometry packing (flag 0x01) â€” real compression â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// NIFSpec.js has always had a decoder for this format (see NIFReader.getGeometry,
// flag 0x01) but no encoder existed anywhere â€” every .nif this app has produced
...
// â”€â”€ Public: packed geometry blob only (no chunk wrapper) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// For the on-demand print-export path (pipeline.py's run_mesh_only): a small
// standalone file containing ONLY [flag][count][bbox?][points...] â€” no .nif
...
// â”€â”€ Public: build a real .nif ArrayBuffer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
/**
 * @param {object} opts
 *   geometry      Float32Array  already-packed 14-floats/point data (optional â€”
...
export function encodeNif(opts = {}) {
  ...
  // KEYFRAME_GEO â€” quantized by default (flag 0x01, ~3.3x smaller than raw,
  // imperceptible error â€” see packGeometryQuantized above). Pass
  ...
  // (CAMERAS/DEPTH/ALPHA/LAYER/MESH/PRINT/SEMANTIC_MAP/etc.) â€” pass an array
  ...
  // discarded everything except geometry â€” cameras, depth maps, meshes,
...
// â”€â”€ Public: decode a .nif ArrayBuffer back into render-ready data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
export async function decodeNif(arrayBuffer) {
  ...
  // CALIBRATION/VERIFICATION â€” this is the live app's only decode path, so
  ...
  // already default safely (uncalibrated / null) when the chunk is absent â€”
  // see NIFSpec.js.
  ...
  // KEYFRAME_MESH â€” decodes both formats now: raw struct (0x00, what every
  ...
  // through a wasm module â€” decodeNif already awaits everything else, so
...
// â”€â”€ Draco decoder â€” lazily created, reused across every mesh this session.
...
// that instead â€” DRACOLoader doesn't care where they come from.
...
// been run against a real encoded file â€” I can't execute WebGL/wasm in this
// environment to confirm it. If colors come through wrong (black, or the
// wrong scale) on the first real Draco-flagged file, that mismatch â€” not the
// vertex/index decode, which is the well-trodden path â€” is the first place
// to look.
      ...
      (geometry) => {
        try {
          ...
          if (colAttr) {
            ...
            // raw uint8 depending on encode-side quantization â€” normalize
            ...
            colors = new Uint8Array(positions.length).fill(200);  // no color attr â€” flat warm grey, matches the raw-format fallback in viewer.js
          }
...
// â”€â”€ Decode KEYFRAME_MESH â†’ {positions, colors, faces} typed arrays, or null.
// Layout (see NIF_SPEC_v1.0.md Â§6b / pipeline.py's _extract_mesh docstring):
//   [format_flag:u8]
//   flag 0x00: [n_verts:u32 BE][n_faces:u32 BE]
//              [positions: n_vertsÃ—3Ã—f32 BE][colors: n_vertsÃ—3Ã—u8][faces: n_facesÃ—3Ã—u32 BE]
//   flag 0x01: Draco buffer â€” decoded via _decodeDracoMesh() above.
async function decodeMeshChunk(chunk) {
  if (!chunk) return null;
  try {
    const meshBytes = await decompressChunk(chunk);
    const dv = new DataView(meshBytes.buffer, meshBytes.byteOffset, meshBytes.byteLength);
    const formatFlag = dv.getUint8(0);

    if (formatFlag === 0x01) {
      return await _decodeDracoMesh(meshBytes.subarray(1));
    }
    if (formatFlag !== 0x00) {
      console.warn(`[nif-format] KEYFRAME_MESH has format_flag ${formatFlag} â€” ` +
 ...
 * Added for nif-part-toggle.js's "generate a baked-open variant" flow â€” this
 ...
 *
 * v2 format â€” the indices block (added alongside pipeline.py's split_layers()
...
/** Decode PHYSICS chunk into {bodies, constraints} â€” see NIFHingeAuthor.js for the shape. */
export async function decodePhysics(reader) {
  return decodePhysicsChunk(reader.getChunk(CHUNK.PHYSICS));
}

// â”€â”€ Public: turn canonical 14-float NIF geometry into THREE-ready arrays â”€â”€â”€â”€
/**
...
export function gaussiansToRenderArrays(gaussians) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // data[o+6..9] is the quaternion â€” not yet consumed by the isotropic
...
// â”€â”€ Decoded NIF geometry â†’ 32-byte-row .splat binary â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// needing to re-point the renderer itself â€” the .nif file is still the only
...
// â”€â”€ File download helper (mirrors fumoc-format.js's downloadFumoc) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
